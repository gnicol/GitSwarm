  ...
  module P4
    class Connection
      ...
      def initialize(config = nil)
        base_dir         = File.join(ROOT_PATH, 'p4')
        ENV['P4TICKETS'] = File.join(base_dir, '.p4tickets') if Dir.writable?(base_dir)
        ENV['P4TRUST']   = File.join(base_dir, '.p4trust')   if Dir.writable?(base_dir)
        @p4              = ::P4.new
