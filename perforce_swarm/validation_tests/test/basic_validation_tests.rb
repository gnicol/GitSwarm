...
class BasicValidationTests < SeleniumBaseTest
  def test_login_page_tile
    ...
    @driver.navigate.to(@@config['gitswarm_url'])
  ...
  def test_dashboard_page_tile
    ...
    @driver.navigate.to(@@config['gitswarm_url'])
    @driver.find_element(id: 'user_login').send_keys(@@config['gitswarm_username'])
    @driver.find_element(id: 'user_password').send_keys(@@config['gitswarm_password'])
