...
RSpec.configure do |config|
  ...
  def testdescription(metadata)
    ...
    config.around(:each) do |test|
        ...
        if(!$rspec_pathlist.include?(path) ||
          $rspec_pathlist[path]==test.metadata[:file_path])
          #Unfortunately, multiple tests will result in the same path if the test is not described with an "it" block (this is pretty simple).  
